...
namespace GUI
{
    public partial class frmTheatre : Form
    {
       /* enum TYPE_GHE
        ...
        TYPE_GHE loaiGhe = TYPE_GHE.FA;
        List<List<Ghe>> listGhe;*/
        ...
        private void BtnSeat_Click(object sender, EventArgs e)
        {
               ...
               /* switch (loaiGhe)
                {
                    ...
                    case TYPE_GHE.Couple:
                        {
                            Ghe temp = ConvertToGhe(btnSeat); // lấy cái ghế hiện tại đang chọn
                            try
                            {
                                Button lblGheKeBen = listGhe[temp.Row][temp.Col+1].Main;
                                if (lblGheKeBen.BackColor == Color.White)
                                {
                                    lblGheKeBen.BackColor = Color.Yellow;
                                }
                                else
                                {
                                    if (btnSeat.BackColor == Color.Red || btnSeat.BackColor == Color.Yellow)
                                        btnSeat.BackColor = Color.White;
                                    MessageBox.Show("Ghế số [" + lblGheKeBen.Text + "] đã có người đặt rồi");
                                }
                            }
                            catch (Exception)
                            {
                                MessageBox.Show("Vị trí bạn chọn không đủ 2 chổ ngồi");
                                btnSeat.BackColor = Color.White;
                            }
                            break;
                        }
                    case TYPE_GHE.Family:
                        {
                            Ghe temp = ConvertToGhe(btnSeat); // lấy cái ghế hiện tại đang chọn
                            if (KiemTraDuDieuKienFML(temp, btnSeat))
                            {
                                ToMauCacGheLienQuanFML(temp);
                            }
                            else
                            {
                                btnSeat.BackColor = Color.White;// không tô màu khi không đặt được 
                            }
                            break;
                        }
                    case TYPE_GHE.Friend:
                        {
                            Ghe temp = ConvertToGhe(btnSeat); // lấy cái ghế hiện tại đang chọn
                            if (KiemTraDuDieuKienFr(temp, btnSeat))
                            {
                                ToMauCacGheLienQuanFr(temp);
                            }
                            else
                            {
                                btnSeat.BackColor = Color.White;// không tô màu khi không đặt được 
            ...
            {
                MessageBox.Show("Ghế số [" + btnSeat.Text + "] đã có người mua", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            LoadBill();
        ...
        /*bool KiemTraDuDieuKienFML(Ghe temp, Button seat)
        {
            for (int i = 0; i < 4; i++)
            {
                try
                {
                    Button lblGheKeBen = listGhe[temp.Row][temp.Col + 1 + i].Main;
                    if (lblGheKeBen.BackColor == Color.Yellow || lblGheKeBen.BackColor == Color.Red)
                    {
                        MessageBox.Show("Ghế số [" + lblGheKeBen.Text + "] đã có người đặt rồi");
                        return false;
                    }
                ...
                {
                    MessageBox.Show("Vị trí bạn chọn không đủ 6 chỗ ngồi");
        ...
        void ToMauCacGheLienQuanFML(Ghe temp)
        {
            for (int i = 0; i < 5; i++)
            {
                Button lblGheKeBen = listGhe[temp.Row][temp.Col + 1 + i].Main;
        ...
        //Danh cho Friend
        bool KiemTraDuDieuKienFr(Ghe temp, Button seat)
        {
            for (int i = 0; i < 7; i++)
            {
                try
                ...
                {
                    MessageBox.Show("Vị trí bạn chọn không đủ 8 chỗ ngồi");
                    seat.BackColor = Color.White;
                    return false;
                }
            }
        ...
        void ToMauCacGheLienQuanFr(Ghe temp)
        {
            for (int i = 0; i < 7; i++)
            {
                Button lblGheKeBen = listGhe[temp.Row][temp.Col + 1 + i].Main;
        ...
        Ghe ConvertToGhe(Button seat)
        {
            foreach (var listGheinRow in listGhe)
            {
                foreach (var col in listGheinRow)
                {
                    if (col.Main.Name == seat.Name)
                        return col; // lấy được ghế cần tìm
                }
            }
            return null;
        }*/
        ...
        private void btnPayment_Click(object sender, EventArgs e)
        {
            if (listSeatSelected.Count == 0)
            {
                MessageBox.Show("Vui lòng chọn vé trước khi thanh toán!", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            if (result == DialogResult.OK)
            {
                ...
                {
                    MessageBox.Show("Bạn đã mua vé thành công!", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            RestoreDefault();
            this.OnLoad(new EventArgs());
        ...
        private void btnFreeTicket_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("BẠN CHỈ ĐỔI ĐƯỢC TỐT ĐA [" + listSeatSelected.Count + "] VÉ", "THÔNG BÁO",MessageBoxButtons.OK,MessageBoxIcon.Warning);
                return;
            }
            int pointFreeTicket = freeTickets * 20;
            if (customer.Point < pointFreeTicket)
            {
                MessageBox.Show("BẠN KHÔNG ĐỦ ĐIỂM TÍCH LŨY ĐỂ ĐỔI [" + freeTickets + "] VÉ", "THÔNG BÁO",MessageBoxButtons.OK,MessageBoxIcon.Information);
                return;
            }
            else
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    {
                        MessageBox.Show("BẠN ĐÃ DỔI ĐƯỢC [" + freeTickets + "] VÉ MIỄN PHÍ THÀNH CÔNG", "THÔNG BÁO",MessageBoxButtons.OK,MessageBoxIcon.Information);
                    }
                    lblPoint.Text = "" + customer.Point;
                    lblPlusPoint.Text = "" + plusPoint;
...
{
   loaiGhe = TYPE_GHE.Couple;
}

private void btnGroup_Click(object sender, EventArgs e)
{
   loaiGhe = TYPE_GHE.Friend;
}

private void btnFamily_Click(object sender, EventArgs e)
{
   loaiGhe = TYPE_GHE.Family;
}
